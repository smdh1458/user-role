...
const PostList = () => {
    ...
    const [keyword, setKeyword] = useState(""); //keyword는 단순히 검색어 이기 때문에 배열 XXX
    ...
    const handleChange = (e) => { // input 에 어떤 이벤트가 발생할 경우 작동하는 기능
        const value = e.target.value.trim(); // 이벤트가 발생한 input 에 들어있는 값을 공백제거하고 value 변수 이름에 담아두기
        setKeyword(value);

        if (value) {
            apiService.suggestedPosts(value, setSugs, setErr);
            setShow(true);
        }
    ...
    *           200(0.2초) 후에 키워드에 따른 추천 검색 리스트 제안을 종료(setTimeout)
    ...
    * */
    return (
        <div>
            ...
            <div>
                ...
                {show && sugs.length > 0 && (
                    <ul>
                        {sugs.map(
                            (sug, index) => (
                                <li key={index} onMouseDown={() => handleSug()}>
                                    {sug}
                                </li>
                            )
                        )}
            ...
            <Link to={`/posts/create`}>추가하기</Link>
        </div>
    );
}
